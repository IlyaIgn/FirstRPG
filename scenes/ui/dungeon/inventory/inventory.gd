...
@onready var unique_attack_node: Node2D = $Sprite2D/UniqueAttack
...
func _ready() -> void:
	if attack_card:
		...
		simple_attack_node.add_child(regular_attack_card_instance)
		...
		if unique_attack:
			...
			unique_attack_node.add_child(unique_attack_card_instance)
